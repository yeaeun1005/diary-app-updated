...
BASE=ROOT/'renders/releases/final-polish-20260922';OUT=ROOT/'renders/releases/audio-type-20260922'
bm=json.loads((BASE/'manifest.json').read_text());assert bm['version']=='db4aca2e5a1b'
sha=lambda b:hashlib.sha256(b).hexdigest()
SOURCE_COMMIT='684227b'
...
for n in ['sea-view.js','passages.js']:files[n]=(V4/n).read_bytes()
files['legacy/landing.js']=(V4/'home/landing.js').read_bytes()
files['guides.js']=(V4/'final-polish/guides.js').read_bytes()
for n in ['sound.js','typography.css']:files[n]=(HERE/n).read_bytes()
for p in (HERE/'assets').iterdir():files[('sounds/' if p.suffix=='.mp3' else 'fonts/')+p.name]=p.read_bytes()
...
s=transformed.replace('__AUDIO_TYPE__',asset).replace(old+'/',asset+'/').replace('content="'+bm['version']+'"','content="'+version+'"')
...
(preview/'__audio-probe.js').write_bytes((HERE/'probe.js').read_bytes())
...
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');print(json.dumps({'version':version,'files':len(manifest['files']),'preview':'http://127.0.0.1:8794/','deployed':False}))
