...
const out='renders/releases/audio-type-20260922/',m=JSON.parse(fs.readFileSync(out+'manifest.json')),page=fs.readFileSync(out+'candidate/index.html','utf8'),src=out+'candidate/'+m.assetPath+'/';
...
for(const name of ['start','login','tut','boat'])S.play(name);assert.equal(Object.keys(S.els).length,0);assert.equal(played.length,0);
...
S.play('boat');assert(S.els.tut.paused);S.set(false);assert(Object.values(S.els).every(a=>a.paused));const n=played.length;S.play('start');assert.equal(played.length,n);
...
const files=JSON.parse(fs.readFileSync('prototypes/island-world-v2/v4/audio-type/ASSETS.json'));
...
console.log(JSON.stringify({pass:true,checks:['muted default creates no audio','four supplied originals mapped','switching effects stops overlap','repeat tutorial restarts at original speed','mute stops active effects','original audio and font hashes','preview instrumentation absent from production','numbered guide punctuation']}));
