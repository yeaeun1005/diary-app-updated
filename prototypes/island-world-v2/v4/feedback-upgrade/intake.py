"""Preserve three user GLBs; create embedded 1K runtime copies without geometry edits."""
...
def module(name, path):
    ...
    return obj
glb = module('feedback_glb', HERE/'glb-tools.py')
inspect = glb
SOURCES = {'talk-mailbox':'cartoon mailbox','tree-willow':'willow tree','tree-bonsai':'stylized bonsai tree'}
def main():
    records=[]
    for asset, name in SOURCES.items():
        ...
        preserved=HERE/'model-source'/(asset+'.glb'); preserved.parent.mkdir(parents=True,exist_ok=True)
        ...
        with tempfile.TemporaryDirectory(prefix='island-feedback-models-') as tmp:
        ...
        dest=HERE/'model-runtime'/(asset+'.glb'); dest.parent.mkdir(parents=True,exist_ok=True); dest.write_bytes(data)
    ...
    (HERE/'models-manifest.json').write_text(json.dumps(records,ensure_ascii=False,indent=2)+'\n')
    print(json.dumps([{'id':r['assetId'],'model-source':r['sourceBytes'],'model-runtime':r['bytes'],'triangles':r['triangles']} for r in records],indent=2))
