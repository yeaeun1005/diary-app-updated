...
OUT=ROOT/'renders/releases/island-feedback-models-20260920'
...
def get(url):
    return subprocess.run(['curl','--fail','--silent','--show-error','--location','--proto','=https','--max-time','40',url],capture_output=True,check=True).stdout
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
    files=list(pool.map(verify,manifest['files'].items()))
