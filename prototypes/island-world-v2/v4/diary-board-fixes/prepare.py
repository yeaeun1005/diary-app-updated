...
BASE=ROOT/'renders/releases/student-refinements-20260921';OUT=ROOT/'renders/releases/diary-board-fixes-20260921'
bm=json.loads((BASE/'manifest.json').read_text());assert bm['version']=='61b6f13bb407'
sha=lambda b:hashlib.sha256(b).hexdigest()
SOURCE_COMMIT='7b17962'
...
files['reward-settings.js']=(V4/'exploration-upgrade/reward-settings.js').read_bytes()
...
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');print(json.dumps({'version':version,'files':len(manifest['files']),'preview':'http://127.0.0.1:8792/','deployed':False}))
