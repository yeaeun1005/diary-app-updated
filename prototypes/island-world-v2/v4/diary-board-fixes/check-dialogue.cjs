...
const html=fs.readFileSync('renders/releases/diary-board-fixes-20260921/candidate/index.html','utf8'),out='renders/releases/diary-board-fixes-20260921/',m=JSON.parse(fs.readFileSync(out+'manifest.json')),src=out+'candidate/'+m.assetPath+'/';
...
(async()=>{
 ...
 await click('안됐어.');await fill('다음에는 어떻게 해볼까요?','다음에는 차분하게 부탁해 볼게.');
 ...
 assert.equal(saved.observation,'발표할 때 친구 두 명이 내 쪽을 봤어.');assert.equal(saved.feeling,'설렘');assert.equal(saved.helpful,'안됐어.');assert.equal(saved.nextPlan,'다음에는 차분하게 부탁해 볼게.');assert.equal(saved.step,7);assert.equal(saved.completed,true);assert.deepEqual(entry,before);assert.equal(db['chat/a'][0].entryTs,2);
 ...
 props={...props,entries:[entry,{...entry,ts:3,text:'친구와 다퉜다. 슬펐다.'}]};await render();let select=nodes().find(n=>n.type==='select');select.props.onChange({target:{value:'3'}});await render();assert.notEqual(nodes().find(n=>n.type==='textarea').props.value,saved.observation);
 ...
 // A positive-only account never silently starts a personal or fictional conversation.
 cells=[];readFail=false;props={...props,me:{id:'positive-only',schoolCode:'TEST'},entries:[{ts:99,date:'2026-09-20',text:'나는 행복했다.',analysis:{}}],data:{selectedEntryTs:99,choices:{},talk:{}}};await render();assert(!nodes().some(n=>n.type==='textarea'));assert(nodes().some(n=>n.props.title==='지금은 대화할 부정 감정 일기가 없어요'));assert(!db['chat/positive-only']);
 // Existing manual corrections prefill the actual chosen emotion after a fresh mount.
 cells=[];props={...props,me:{id:'corrected',schoolCode:'TEST'},entries:[{ts:99,date:'2026-09-20',text:'나는 행복했다.',analysis:{corrected:true,hits:[{label:'슬픔',val9:2}]}}]};await render();await click('02느낌');assert.equal(nodes().find(n=>n.type==='textarea').props.value,'슬픔');
