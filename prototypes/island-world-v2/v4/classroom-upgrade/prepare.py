...
BASE=ROOT/'renders/releases/mind-dialogue-20260920';OUT=ROOT/'renders/releases/classroom-controls-20260920'
bm=json.loads((BASE/'manifest.json').read_text());assert bm['version']=='3739dea5bb1f'
sha=lambda b:hashlib.sha256(b).hexdigest()
SOURCE_COMMIT='34a6cc8'
...
for n in ['activities.js','student-tools.js','scene.js','friend-scene.js','friend-view.js','sea-view.js']:files[n]=(V4/n).read_bytes()
...
files['legacy/board.js']=(HERE/'board.js').read_bytes()
files['legacy/landing.js']=(V4/'home/landing.js').read_bytes()
for name in ['care-model.js','teacher-tools.js','classroom.css']:files[name]=(HERE/name).read_bytes()
from transform import apply
transformed=apply(source)
transformed=rep(transformed,'<script>root.render(React.createElement(App));</script>','<script src="__CLASSROOM__/care-model.js"></script><script src="__CLASSROOM__/teacher-tools.js"></script><link rel="stylesheet" href="__CLASSROOM__/classroom.css"><script>root.render(React.createElement(App));</script>')
...
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');print(json.dumps({'version':version,'files':len(manifest['files']),'preview':'http://127.0.0.1:8788/','deployed':False}))
