...
const out='renders/releases/'+(process.argv[2]||'student-refinements-20260921')+'/',m=JSON.parse(fs.readFileSync(out+'manifest.json')),source=fs.readFileSync(out+'before-work/index.html','utf8'),candidate=fs.readFileSync(out+'candidate/index.html','utf8'),preview=fs.readFileSync(out+'memory-preview/index.html','utf8'),assets=out+'candidate/'+m.assetPath+'/',hash=x=>crypto.createHash('sha256').update(x).digest('hex');
