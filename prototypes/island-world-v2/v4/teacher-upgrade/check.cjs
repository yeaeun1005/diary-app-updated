...
const out='renders/releases/login-teacher-upgrade-20260919/',m=JSON.parse(fs.readFileSync(out+'manifest.json')),base=fs.readFileSync(out+'before-work/index.html','utf8'),page=fs.readFileSync(out+'candidate/index.html','utf8'),preview=fs.readFileSync(out+'memory-preview/index.html','utf8');
const cut=(s,a,b)=>{const x=s.indexOf(a),y=s.indexOf(b,x);assert(x>=0&&y>x,a);return s.slice(x,y);};
for(const[a,b]of [['  const teacherLogin = async () => {','  // 학생 로그인 본체.'],['  const studentLoginWith = async','  const studentLogin = () =>'],['  const syncIsland = useCallback','  const logout ='],['async function d2Put','// ── ⑥ 바다']])assert.equal(cut(page,a,b),cut(base,a,b));
assert.equal(cut(page,'function dbRef(k)','// 실패를 그대로 던진다.').replace('  if(typeof V4ReviewClass!=="undefined" && V4ReviewClass.active())return V4ReviewClass.ref(k);\n',''),cut(base,'function dbRef(k)','// 실패를 그대로 던진다.'));
...
const oldAsset='island-v4/'+/island-world-release" content="([^"]+)/.exec(base)[1];
for(const f of Object.keys(m.files).filter(f=>f.startsWith(m.assetPath+'/'))){const prev=f.replace(m.assetPath,oldAsset);if(fs.existsSync(prev))assert.equal(crypto.createHash('sha256').update(fs.readFileSync(prev)).digest('hex'),m.files[f],'Previous student/home asset changed: '+f);}
let realCalls=0;
...
(async()=>{
 ...
 console.log(JSON.stringify({pass:true,version:m.version,files:Object.keys(m.files).length,checks:['original real teacher/student auth preserved','all original student/home assets unchanged','PDF included and teacher labels updated','isolated Firebase-stripped preview','fictional teacher and student share DEMO01 and 25 students','student write visible after teacher role switch','zero real DB references during review','normal account uses original gateway','invalid memory paths rejected']},null,2));
