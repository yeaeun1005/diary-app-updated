...
OUT=ROOT/'renders/releases/review-polish-20260922'
manifest=json.loads((OUT/'manifest.json').read_text())
assert manifest['previousCandidate']=='1877d3db2428'
for name,digest in manifest['files'].items():
    tracked=subprocess.check_output(['git','show','HEAD:'+name],cwd=ROOT)
    assert hashlib.sha256(tracked).hexdigest()==digest, 'Committed file differs: '+name
