...
const html=fs.readFileSync('index.html','utf8'),out='renders/releases/mind-dialogue-20260920/',m=JSON.parse(fs.readFileSync(out+'manifest.json')),src=out+'candidate/'+m.assetPath+'/';
...
for(const code of[cut('function normalizeKorean(t)','function safeAnalysis(a)'),cut('const SITUATION_MAP =','function MindChat({'),cut('function extractSituations(textArray)','function fmtDate(d)'),'function v4Analyze(t){return analyzeEntry(t)}',fs.readFileSync(src+'activity-model.js','utf8'),fs.readFileSync(src+'student-tools-model.js','utf8'),fs.readFileSync(src+'content.js','utf8'),fs.readFileSync(src+'activities.js','utf8')])vm.runInContext(code,ctx);
