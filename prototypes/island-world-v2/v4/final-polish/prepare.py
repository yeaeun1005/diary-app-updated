...
BASE=ROOT/'renders/releases/diary-board-fixes-20260921';OUT=ROOT/'renders/releases/final-polish-20260922'
bm=json.loads((BASE/'manifest.json').read_text());assert bm['version']=='7a8468f313e6'
sha=lambda b:hashlib.sha256(b).hexdigest()
SOURCE_COMMIT='882815a'
...
for n in ['activities.js','student-tools.js','sea-view.js','sea-scene.js','friend-view.js']:files[n]=(V4/n).read_bytes()
app=(V4/'app.js').read_text();app=app[app.index('const v4h='):]
for a,b in [('function V4App(props){','function V4AppInner(props){'),('window.__V4=api.current;',''),('delete window.__V4;',''),("'로컬 검수용 섬  새로고침하면 연습 기록이 초기화돼요.'","MEM_ONLY?'체험 기록은 새로고침하면 사라져요.':'이 계정에 일기와 탐험 기록을 저장해요.'")]:app=rep(app,a,b)
files['app.js']=app.encode()
files['sharing.js']=(V4/'social-upgrade/sharing.js').read_bytes()
files['legacy/board.js']=(V4/'classroom-upgrade/board.js').read_bytes()
files['scene.js']=(V4/'scene.js').read_text().replace('const V4_QUERY=new URLSearchParams(location.search)','const V4_QUERY=new URLSearchParams()').encode()
for n in ['tutorial.js','tutorial.css']:files[n]=(V4/'model-upgrade'/n).read_bytes()
for n in ['guides.js','polish.css','review-class.js']:files[n]=(HERE/n).read_bytes()
...
s=transformed.replace('__POLISH__',asset).replace(old+'/',asset+'/').replace('content="'+bm['version']+'"','content="'+version+'"')
...
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');print(json.dumps({'version':version,'files':len(manifest['files']),'preview':'http://127.0.0.1:8793/','deployed':False}))
