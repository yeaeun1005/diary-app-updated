...
from html_changes import apply
HERE=Path(__file__).resolve().parent
V4=HERE.parent
ROOT=HERE.parents[3]
OUT=ROOT/'renders/releases/mind-island-v4-20260919'
backup=OUT/'before-work/index.html'
SOURCE=backup.read_text() if backup.exists() else subprocess.check_output(['git','show','6e694fefe3d6b87da12982db3ec1e0550cffae64:index.html'],cwd=ROOT,text=True)
sha=lambda data:hashlib.sha256(data).hexdigest()
def rep(s,a,b):
 assert s.count(a)==1, 'Changed integration boundary: '+a[:80]
 return s.replace(a,b)
sys.path.insert(0,str(ROOT/'prototypes/island-world-v2/v3_1'))
from labels import apply_labels
old='island-v2/502991c9f22e'
files={}
for p in (ROOT/old).iterdir():
 if p.is_file():
  data=p.read_bytes()
  if p.name in ('world.js','board.js','landing.js'):data=apply_labels(data.decode(),p.stem)[0].encode()
  files['legacy/'+p.name]=data
files['GLTFLoader.r147.js']=(HERE/'vendor/GLTFLoader.r147.js').read_bytes()
modules=['state','content','assets','passages','scene','sea-scene','sea-view','friend-style','friend-scene','friend-view','persistence','app']
for name in modules:
...
for name in ['style.css','sea.css','friend.css']:files[name]=(V4/name).read_bytes()
...
page=apply(apply_labels(SOURCE,'html')[0])
version=sha(page.encode()+b''.join(k.encode()+files[k] for k in sorted(files)))[:12]
asset_path='island-v4/'+version
page=page.replace(old,asset_path+'/legacy')
page=rep(page,'<meta name="island-world-release" content="502991c9f22e">','<meta name="island-world-release" content="'+version+'">')
tags=''.join('<link rel="stylesheet" href="'+asset_path+'/'+n+'">\n'for n in ['style.css','sea.css','friend.css'])
...
manifest={'version':version,'assetPath':asset_path,'previousCommit':'6e694fefe3d6b87da12982db3ec1e0550cffae64','sourceSha256':sha(SOURCE.encode()),'candidateSha256':sha(page.encode()),'databaseMigration':False,'deployed':False,'files':{'index.html':sha(page.encode()),**{asset_path+'/'+k:sha(v) for k,v in files.items()}},'bytes':sum(len(v) for v in files.values())+len(page.encode())}
...
print(json.dumps({'version':version,'files':len(manifest['files']),'bytes':manifest['bytes'],'memoryPreview':'http://127.0.0.1:8776/','productionExecuted':False},indent=2))
