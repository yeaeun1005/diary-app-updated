...
const out='renders/releases/'+(process.argv[2]||'mind-island-v4-20260919')+'/',m=JSON.parse(fs.readFileSync(out+'manifest.json')),source=fs.readFileSync(out+'before-work/index.html','utf8'),candidate=fs.readFileSync(out+'candidate/index.html','utf8'),preview=fs.readFileSync(out+'memory-preview/index.html','utf8'),assets=out+'candidate/'+m.assetPath+'/',hash=x=>crypto.createHash('sha256').update(x).digest('hex');
...
if(fs.existsSync(assets+'decor-model.js')){
 for(const name of ['decor-model.js','decor-scene.js','decor-ui.js','decor.css','activity-model.js','activities.js','activities.css'])assert(Object.hasOwn(m.files,m.assetPath+'/'+name),'Missing new module '+name);
 const inline=html=>[...html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)].map(x=>x[1]).filter(x=>x.trim());
 assert.deepEqual(inline(candidate),inline(source),'Published inline application code changed');
 const order=[...candidate.matchAll(/<script src="([^"]+)"/g)].map(x=>x[1]);assert(order.indexOf(m.assetPath+'/decor-model.js')<order.indexOf(m.assetPath+'/state.js'));
 assert.equal(Object.keys(m.files).filter(f=>/assets\/models\/decor-.+-data\.js$/.test(f)).length,6);
}
const before=JSON.parse(fs.readFileSync(out+'protected-before.json'));for(const[p,h]of Object.entries(before)){const actual=hash(fs.readFileSync(p));if(p==='index.html'&&actual===m.candidateSha256)continue;assert.equal(actual,h,'Preserved source '+p);}
