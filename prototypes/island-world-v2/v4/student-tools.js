...
function V4AnalysisTools({entries}){
 const [period,setPeriod]=useState('all'),[scope,setScope]=useState('all'),[start,setStart]=useState(''),[end,setEnd]=useState(''),[hover,setHover]=useState(null);
 ...
 return v4h('section',{className:'v4-restored-analysis','aria-label':'나의 감정 분석'},
  v4h('div',{className:'v4-restored-card'},v4h('h3',null,'기록 속 마음을 살펴봐요'),
   v4h('div',{className:'v4-segment','aria-label':'분석 기간'},[['7','최근 7일'],['30','최근 30일'],['all','전체'],['custom','직접 입력']].map(([id,label])=>v4h('button',{key:id,'aria-pressed':period===id,onClick:()=>setPeriod(id)},label))),
   ...
   v4h('label',{className:'v4-field'},v4h('span',null,'분석할 일기'),v4h('select',{value:scope,onChange:e=>{setScope(e.target.value);setHover(null);}},v4h('option',{value:'all'},'기간 안의 모든 일기'),ordered.map(e=>v4h('option',{key:e.ts,value:String(e.ts)},e.date+' · '+e.text.slice(0,28))))),
  ...
  !filtered.length?v4h(V4Empty,{title:'이 범위에는 아직 일기가 없어요',copy:'다른 기간을 고르거나 감정일기를 먼저 남겨 보세요.'}):v4h(React.Fragment,null,
   v4h('section',{className:'v4-restored-card'},v4h('h4',null,'긍정·부정 감정 분석 그래프'),v4h('p',{className:'v4-fine'},'왼쪽은 부정, 오른쪽은 긍정 · 위아래는 에너지의 높고 낮음이에요. 좋은 감정과 나쁜 감정을 나누는 점수는 아니에요.'),
    v4h(CircumplexChart,{entries:summary.chartEntries,hovIdx:hover,onHover:setHover,teacher:false}),
    v4h('div',{className:'v4-chips'},summary.chartEntries.map((e,i)=>v4h('button',{key:e.ts,'aria-pressed':hover===i,onClick:()=>setHover(hover===i?null:i),onMouseEnter:()=>setHover(i),onMouseLeave:()=>setHover(null)},e.date+' 일기 '+(i+1))))),
   ...
   v4h('section',{className:'v4-restored-card'},v4h('h4',null,'상황 탐지'),v4h('p',null,'일기에 자주 나타난 상황이에요. 단어를 바탕으로 찾았으니 실제 경험과 다를 수 있어요.'),summary.situations.length?v4h('div',{className:'v4-word-tags'},summary.situations.map(s=>v4h('span',{key:s.text},s.text+' · '+s.count+'편'))):v4h('p',null,'자동으로 찾은 상황이 없어요.')),
   ...
   v4h('section',{className:'v4-restored-card'},v4h('h4',null,'긍정·부정 감정 변화'),summary.trendEntries.length>=2?v4h(TrendChart,{entries:summary.trendEntries}):v4h('p',null,'감정 표현을 찾은 일기가 두 편 이상이면 변화를 보여줘요.'))));
}
function V4TalkDetection({text,chosen,feeling,onFeeling,disabled}){
 const found=useMemo(()=>V4StudentTools.detected(text),[text]);
 const options=[...new Set([...chosen,...found.words,...found.suggested.map(e=>e.label)])];
 return v4h('section',{className:'v4-talk-detection','aria-label':'자동 감정·상황 탐지'},v4h('h4',null,'자동 감정·상황 탐지'),
  v4h('p',null,'감정: '+(found.words.join(' · ')||'찾은 말이 없어요. 직접 표현해도 괜찮아요.')),
  v4h('p',null,'상황: '+(found.situations.map(s=>s.ctx).join(' · ')||'찾은 상황이 없어요.')),
  options.length>0&&v4h(React.Fragment,null,v4h('p',{className:'v4-fine'},'아래 말 중 내 마음에 가까운 것을 골라 대화에 넣을 수 있어요.'),v4h('div',{className:'v4-chips'},options.map(w=>v4h('button',{key:w,type:'button',disabled,onClick:()=>onFeeling(feeling?([...new Set([...feeling.split(' · '),w])].join(' · ')):w)},w)))),
  v4h('p',{className:'v4-fine'},'자동 탐지는 힌트예요. 선택한 마음과 원래 일기는 임의로 바뀌지 않아요.'));
}
function V4ActionSuggestions({text,value,onChoose,disabled}){
 const options=useMemo(()=>V4StudentTools.actionExamples(detectSituations(text)),[text]);
 return v4h('section',{className:'v4-action-suggestions'},v4h('h5',null,'행동 기술 예시 추천'),v4h('p',{className:'v4-fine'},'나의 느낌 말하기 · 부탁하기 · 잠시 멈추기 · 도움 요청하기. 내 상황에 맞는 예시를 골라 고쳐 써도 좋아요.'),
  v4h('div',{className:'v4-suggestion-list'},options.map(s=>v4h('button',{key:s,type:'button',disabled,'aria-pressed':value===s,onClick:()=>onChoose(s)},s))));
}
