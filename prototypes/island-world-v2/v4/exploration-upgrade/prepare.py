"""Build the classroom controls/teacher choices/home update; no DB/network."""
from pathlib import Path
import json,hashlib,re,shutil,zipfile,subprocess
HERE=Path(__file__).resolve().parent;V4=HERE.parent;ROOT=HERE.parents[3]
BASE=ROOT/'renders/releases/classroom-controls-20260920';OUT=ROOT/'renders/releases/daily-exploration-20260920'
bm=json.loads((BASE/'manifest.json').read_text());assert bm['version']=='d462f3a7c9b0'
sha=lambda b:hashlib.sha256(b).hexdigest()
SOURCE_COMMIT='c256743'
...
state=(V4/'state.js').read_text().replace("throw Error('local-adapter-required')","throw Error('adapter-required')").replace("const state=reduce(load(me),event);await adapter.write(k,state);return state;","if(adapter.dispatch)return adapter.dispatch(k,event);const state=reduce(load(me),event);await adapter.write(k,state);return state;")
files['state.js']=state.encode()
for name in ['daily-model.js','reward-settings.js','exploration.css']:files[name]=(HERE/name).read_bytes()
from transform import apply
transformed=apply(source)
transformed=rep(transformed,'<script>root.render(React.createElement(App));</script>','<script src="__CLASSROOM__/daily-model.js"></script><script src="__CLASSROOM__/reward-settings.js"></script><link rel="stylesheet" href="__CLASSROOM__/exploration.css"><script>root.render(React.createElement(App));</script>')
...
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');print(json.dumps({'version':version,'files':len(manifest['files']),'preview':'http://127.0.0.1:8789/','deployed':False}))
