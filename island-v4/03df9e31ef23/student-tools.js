...
function V4WritingHelper({text,emotions,situations,onEmotions,onSituations,disabled}){
 ...
 return v4h('section',{className:'v4-restored-card','aria-label':'감정과 상황 도움받기'},
  v4h('h4',null,'감정과 상황 도움받기'),v4h('p',null,'글이 잘 떠오르지 않으면 마음과 상황부터 골라 봐요. 여러 개를 골라도 괜찮아요.'),
...
function V4AnalysisTools({entries,initialEntryTs=null}){
 ...
 return v4h('section',{className:'v4-restored-analysis','aria-label':'나의 감정 분석'},
  v4h('div',{className:'v4-restored-card'},v4h('h3',null,'기록 속 마음을 살펴봐요'),v4h('p',{className:'v4-emotion-guide'},'나의 원하는 바가 이루어졌을 때 우리는 긍정적인 감정(예: 기쁨)을 느껴요. 부정적인 감정(예: 슬픔)은 내가 원하는 것이 있다는 신호에요.'),
   ...
   v4h('label',{className:'v4-field'},v4h('span',null,'분석할 일기'),v4h('select',{className:'v4-analysis-scope'+(scope==='all'?' is-all':''),'aria-label':'분석할 일기',value:scope,onChange:e=>{setScope(e.target.value);setHover(null);setFocus(null);}},v4h('option',{value:'all'},'기간 안의 모든 일기'),ordered.map(e=>v4h('option',{key:e.ts,value:String(e.ts)},e.date+'  '+e.text.slice(0,28))))),
  ...
  !filtered.length?v4h(V4Empty,{title:'이 범위에는 아직 일기가 없어요',copy:'다른 기간을 고르거나 감정일기를 먼저 남겨 보세요.'}):v4h(React.Fragment,null,
   v4h('section',{className:'v4-restored-card'},v4h('h4',null,'긍정과 부정 감정 변화'),v4h(V4AnalysisHelp,null,'감정일기에 나타난 내 마음은 어떤 특성을 보이나요?'),summary.trendEntries.length>=2?v4h(TrendChart,{entries:summary.trendEntries,readable:true}):v4h('p',null,'감정 표현을 찾은 일기가 두 편 이상이면 변화를 보여줘요.')),
