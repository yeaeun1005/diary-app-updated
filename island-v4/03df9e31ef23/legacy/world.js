...
const V2_LOCAL=typeof __DB==='object';
...
function v2BoardNote(){return MEM_ONLY?'체험 중':V2_LOCAL?'로컬 연결 검증 · 새로고침하면 연습 기록이 사라져요.':'친구의 이름을 쓰지 않고, 마음을 함께 나눠요.';}
...
function V2World({state,onOpen,panel,me,profile=me,friend=null,myState=state,onLogout,onHidden,trial=false}){
  ...
  useEffect(()=>{try{api.current=v2CreateWorld(host.current,state,{open:a=>openRef.current(a),update:setUI,motion:setMotion},V2_SESSIONS[profile.id],{friend,islandId:profile.id,quality});if(V2_LOCAL)window.__V2=api.current;return()=>{V2_SESSIONS[profile.id]=api.current.snapshot();api.current.dispose();if(V2_LOCAL)delete window.__V2;};}catch(e){console.error('V2 renderer:',e);setFailed('3D 화면을 열지 못했어요. 아래 바로가기를 이용해 주세요.');}},[state,profile.id,quality]);
  ...
  async function send(){const p=ui.nearGrowth;if(!p||sending||!socialReady)return;setSending(true);setNotice('');try{const result=await v2SendEmpathy(me,friend,p.label);if(!alive.current)return;if(result.status==='sent-now'){setJustSent(m=>({...m,[dayStamp()+':'+p.label]:true}));setNotice('작은 반딧불에 마음을 담아 보냈어요.');api.current?.flyEmpathy(p,()=>{if(alive.current)setRecords(result.records);});}else setNotice(result.status==='sent'?'이 마음에는 오늘 이미 보냈어요.':result.status==='private'?'지금은 공개되지 않은 마음이에요.':'같은 쪽 마음을 기록해 본 뒤에 보낼 수 있어요.');}catch(e){if(alive.current)setNotice('보내지 못했어요. 잠시 후 다시 눌러 주세요.');}finally{if(alive.current)setSending(false);}}
  async function hide(){const p=ui.nearGrowth;if(!p||sending)return;setSending(true);try{const hidden=!(state.find(e=>e.label===p.label)?.hidden);await isleHide(me,p.label,hidden);if(alive.current){onHidden?.(p.label,hidden);setNotice(hidden?'내 섬에는 그대로, 친구에게는 보이지 않아요.':'친구에게 다시 보이는 마음이에요.');}}catch(e){if(alive.current)setNotice('변경하지 못했어요. 다시 눌러 주세요.');}finally{if(alive.current)setSending(false);}}
  ...
  return v2h('main',{className:'v2-world'+(motion?' v2-busy':'')},
    ...
    v2h('div',{className:'v2-local'},v2h('i'), v2EnvironmentLabel(me)),
    v2h('nav',{className:'v2-shortcuts','aria-label':'마음 활동 바로가기'},!isFriend&&button('✎  내 안으로',null,()=>onOpen('diary')),button('▤  우리 반 게시판',null,()=>onOpen('board')),button('⚑  부두로 걷기',null,()=>api.current?.walkTo(V2.stops.dock)),!isFriend&&onLogout&&button(V2_LOCAL?'처음으로':trial?'체험 끝내기':'로그아웃',null,onLogout)),
    !ui.zone.key&&v2h('section',{className:'v2-location'},v2h('span',{className:'v2-eyebrow'},'오늘의 산책'),v2h('h2',null,ui.zone.name),v2h('p',null,ui.zone.sub)),
    ...
    mapOpen&&v2h('aside',{className:'v2-map'},v2h('div',{className:'v2-map-title'},isFriend?'친구 섬 지도':'나의 섬 지도',v2h('small',null,'구역은 섬에 고정돼요')),
      v2h('svg',{viewBox:'0 0 140 118','aria-label':'섬의 길과 연못, 현재 위치'},
    ...
    guide&&v2h('div',{className:'v2-guide'},v2h('strong',null,'어디로 걸어갈까요?'),[...Object.entries(V2.stops).map(([key,s])=>key==='garden'?{...s,title:'→ 걷기',hint:'지도 오른쪽 길로 걷기'}:s),...V2.zones.map((z,i)=>({...z,title:['↑ 걷기','→ 걷기','↓ 걷기','← 걷기'][i],hint:['지도 위쪽으로 걷기','지도 오른쪽으로 걷기','지도 아래쪽으로 걷기','지도 왼쪽으로 걷기'][i]}))].map(s=>button(s.title,s.hint,()=>{api.current?.walkTo(s);setGuide(false);})),state.length>0&&button('마음이 자란 곳으로 걷기',null,()=>{if(!api.current?.walkToGrowth())setNotice('주변의 마음 식물에 가까이 걸어가 보세요.');setGuide(false);})),
    v2h('div',{className:'v2-controls'},v2h('div',{className:'v2-camera'},button('↶','시점 왼쪽 회전',()=>api.current?.rotate(-Math.PI/4)),button('↷','시점 오른쪽 회전',()=>api.current?.rotate(Math.PI/4)),v2h('span',{className:'v2-divider'}),button('−','축소',()=>api.current?.zoom(2)),button('+','확대',()=>api.current?.zoom(-2)),button('⌂  기본 시점',null,()=>api.current?.reset()),v2h(SndBtn,{small:true,className:'v2-sound'}),button('?','조작 도움말',()=>setHelp(true)))),
