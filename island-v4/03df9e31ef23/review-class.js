 ...
 async function start(){
  if(!prepared)prepared=(async()=>{
   ...
   for(const s of generated.students){
    ...
    if(s.loginId===forms.student.loginId){
     const ts=exampleTs(new Date(DEMO_CLOCK+'T15:00:00').getTime()),text='쉬는 시간에 친구들이 나를 빼고 놀이를 시작했다. 함께 놀고 싶었는데 속상하고 외로웠다. 나도 같이 놀고 싶다고 친구에게 말했다.';
     diary[entryKey(ts)]={ts,date:fmtDate(new Date(DEMO_CLOCK+'T12:00:00')),text,analysis:{...analyzeEntry(text),entryDate:DEMO_CLOCK}};
    }
    put(d2Key(s.id),diary);
    // Fictional examples exist only in this tab's in-memory review class.
    const shared={};
    for(const e of Object.values(diary).filter(e=>e&&typeof e.text==='string').sort((a,b)=>b.ts-a.ts).slice(0,6))shared[entryKey(e.ts)]={public:true,schoolCode:'DEMO01',date:e.date,text:e.text,emotions:[...new Set((e.analysis?.hits||[]).map(h=>h.label))],publishedAt:e.ts};
    put(isleKey(s.id),{...generated.isle[s.id],code:'DEMO01',shared});
