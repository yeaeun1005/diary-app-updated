...
function V4Talk({me,entries,data,change,onLeavePlace,onEarned}){
 const sorted=useMemo(()=>V4ActivityModel.inRange(V4StudentTools.refresh(entries).filter(V4StudentTools.hasNegative),'all',dayStamp()),[entries]);
 ...
 const seed=useMemo(()=>{const s=V4StudentTools.talkSeed(raw);if(entry?.analysis?.corrected)s.feeling=(entry.analysis.hits||[]).map(h=>h.label).join(', ');s.feeling=s.feeling.replaceAll(' · ', ', ');return s;},[raw,entry?.analysis]);
 ...
 async function save(){
  ...
  try{
   ...
   if(fiction){await change({type:'talk',ts:null,text:[...steps.map(s=>s.label+': '+(draft[s.key]||'천천히 생각해 볼게요.')),draft.helpful==='안됐어.'&&draft.nextPlan?'다음에는 어떻게 해볼까요?: '+draft.nextPlan:''].filter(Boolean).join('\n')});}
   else {
 ...
 const summary=record=>v4h('dl',null,steps.map(s=>record[s.key]&&v4h(React.Fragment,{key:s.key},v4h('dt',null,s.label),v4h('dd',null,record[s.key]))),record.nextPlan&&v4h(React.Fragment,null,v4h('dt',null,'다음에는 어떻게 해볼까요?'),v4h('dd',null,record.nextPlan)));
 if(!entry&&!fiction)return v4h(React.Fragment,null,v4h(V4ActivityHeading,{kind:'talk',kicker:'마음 우체통',title:'내 마음을 찬찬히 되돌아봐요.',copy:'왜 이런 감정을 느꼈을까요? 다섯 질문을 통해 찬찬히 돌아봐요.'}),v4h(V4Empty,{title:'지금은 대화할 부정 감정 일기가 없어요',copy:'부정 감정이 담긴 일기를 쓰거나, 최근 일기에서 내 감정을 직접 고르면 여기에서 돌아볼 수 있어요.',onWrite:()=>onLeavePlace('write')}),btn('가상 이야기로 연습하기',()=>choose('fiction'),'v4-text'),completionNext(),error&&v4h('p',{className:'v4-inline-error',role:'alert'},error),chats.length>0&&v4h('details',{className:'v4-history'},v4h('summary',null,'전에 남긴 대화 보기'),chats.map((c,i)=>v4h('article',{key:i},v4h('small',null,c.entryDate),summary(c)))));
 return v4h(React.Fragment,null,
  ...
  v4h('div',{className:'v4-activity-columns'},
   ...
   v4h('div',{className:'v4-work-card v4-talk-work'},
    v4h('nav',{ref:stepNav,className:'v4-step-nav v4-five-steps','aria-label':'마음 대화 순서'},steps.map((s,i)=>v4h('button',{key:s.key,'aria-current':draft.step===i?'step':undefined,onClick:()=>go(i),disabled:busy},v4h('span',null,String(i+1)),s.label))),
    saved?v4h('section',{className:'v4-saved',role:'status'},v4h('span',{className:'v4-saved-mark'},'✓'),v4h('h4',null,'내 마음 대화를 남겼어요'),v4h('p',null,'내가 바랐던 것과 했던 행동을 돌아봤어요. 다음에도 내 마음을 천천히 들어줘요.'),summary(draft),completionNext(),btn('조금 더 생각하기',()=>setSaved(false)),btn('일기 보관함 밝히기',()=>onLeavePlace('archive'),'v4-primary')):
    v4h(React.Fragment,null,
     v4h('section',{className:'v4-talk-question'},v4h('span',{className:'v4-eyebrow'},String(draft.step+1)+' / 5  '+current.label),v4h('h4',{ref:question,tabIndex:-1},current.question),
      ...
      draft.step===4&&v4h(React.Fragment,null,v4h('p',null,'원했던 것과 실제로 한 행동을 나란히 보고, 가장 가까운 답을 골라요.'),v4h('div',{className:'v4-reflection-recap'},v4h('div',null,v4h('small',null,'내가 원했던 것'),v4h('p',null,draft.need||'앞의 욕구 질문에서 적을 수 있어요.')),v4h('div',null,v4h('small',null,'내가 한 행동'),v4h('p',null,draft.action||'앞의 행동 질문에서 적을 수 있어요.'))),v4h('div',{className:'v4-reflection-options','aria-label':'돌아보기 선택'},V4StudentTools.reflections.map(r=>v4h('button',{type:'button',key:r.value,disabled:busy,'aria-pressed':draft.helpful===r.value,onClick:()=>update('helpful',r.value)},v4h('strong',null,r.value)))),draft.helpful==='안됐어.'&&field('nextPlan','다음에는 어떻게 해볼까요?','다음에 해 보고 싶은 행동을 내 말로 적어 보세요.'),draft.helpful&&!V4StudentTools.reflections.some(r=>r.value===draft.helpful)&&v4h('p',{className:'v4-callout'},'전에 남긴 돌아보기: '+draft.helpful))),
...
function V4Practice(){
 ...
 return v4h(React.Fragment,null,
  mode==='choose'?v4h('div',{className:'v4-practice-grid'},options.map(([id,title,desc,tag],i)=>v4h('button',{key:id,className:'v4-practice-card',onClick:()=>setMode(id)},v4h('span',{className:'v4-card-number'},String(i+1)),v4h('strong',null,title),v4h('span',null,desc),v4h('span',{className:'v4-card-arrow','aria-hidden':true},'↗')))):
...
function V4Archive({me,entries,data,change,onLeavePlace,onUpdate,onDelete,onLegacy,readOnly=false,onInspect}){
 ...
 return v4h(React.Fragment,null,v4h(V4ActivityHeading,{kind:'archive',kicker:'기록 보관함  나에게 남긴 이야기',title:'지난 마음을 다시 만나요.',copy:'달력 속 하루를 펼치거나, 여러 날의 기록을 함께 돌아봐요.'}),
  ...
  !readOnly&&v4h(V4OwnSharedDiaries,{me}),!readOnly&&v4h(V4ReceivedMessages,{me}),!readOnly&&v4h('div',{className:'v4-archive-extra'},v4h('button',{className:'v4-text',onClick:onLegacy},'세부 분석과 기록 도구')),
