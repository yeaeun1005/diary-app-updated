...
const V4Decor=(()=>{
 const items={
  ...
  swing:{gift:true,name:'별빛 그네',icon:'☆',desc:'살랑이는 바람과 쉬어 가는 자리',width:2.3,r:1.46,color:'#e8c886'},
  lighthouse:{gift:true,name:'작은 등대',icon:'♜',desc:'마음 바다를 밝히는 포근한 불빛',height:2.4,r:1.10,color:'#e4a99b'},
  flowerBoat:{gift:true,name:'꽃배 화분',icon:'❀',desc:'작은 나무배에 가득 핀 꽃',width:2.1,r:1.20,color:'#d9b17d'}
 };
 const zones={shade:{name:'나무 곁 정원',sub:'차분한 초록 사이, 넓어진 쉼터',x:-4.7,z:-6.45,halfX:3.4,halfZ:3.2},breeze:{name:'산책길 곁 정원',sub:'햇살을 따라 소품을 모아 봐요',x:6,z:2.35,halfX:3.25,halfZ:3.2},meadow:{name:'큰 나무 뒤뜰',sub:'커다란 나무 뒤에 나만의 정원',x:.3,z:-11.8,halfX:4.5,halfZ:3.0},coast:{name:'바다가 보이는 정원',sub:'바다 가까이 펼쳐진 작은 마당',x:11.2,z:10.8,halfX:4.0,halfZ:2.8}};
 zones.sunset={name:'노을빛 꽃마당',sub:'분홍 나무 곁, 넓게 펼쳐진 정원',x:-12,z:-11.5,halfX:4.5,halfZ:3.2};zones.picnic={name:'남쪽 소풍 정원',sub:'산책길 아래 포근한 나만의 자리',x:-3,z:13.0,halfX:4.8,halfZ:3.0};
 const oldSpots={'desk-garden':{x:.2,z:-3.8},'water-garden':{x:9,z:3},'bench-garden':{x:-8,z:1}};
 const obstacles=[[-6.2,-5.3,.4],[3,-9,.65],[-9,-7,.48],[-12,-2,.4],[-6,-10,.4],[9,-10,.4],[14,4,.35],[-16,3,.48],[-14,-8,.45],[-9,11,.4],[17,-8,.45],[17,6,.43],[3,-6.5,1.45],[-8,-5.3,1.15],[-10,5.5,1],[19,1.7,.74],[-3.8,8.7,1.3],[-5,-2,.45],[7,5,.45],[-5,6,.45],[5,-5,.45]].map(([x,z,r])=>({x,z,r}));
 ...
 function catalog(s){const owned=Object.fromEntries(Object.entries(items).filter(([id,item])=>!item.gift||own(s?.decor,id)));const id=s?.reward?.item;if(id)owned.reward=items[id]?{...items[id],modelId:id,gift:true,desc:'마음 탐험 01에서 받은 선물'}:{name:({flower:'도톰한 꽃',lantern:'작은 정원 등불',shrub:'둥근 관목'})[id],icon:'✦',desc:'마음 탐험 01에서 받은 선물',reward:id,r:({flower:.87,lantern:.82,shrub:1.46})[id]};return owned;}
 ...
 function reason(s,id,p){const item=catalog(s)[id];if(!item)return '이 소품을 아직 가지고 있지 않아요.';if(p===null)return '';if(!p||![p.x,p.z,p.rotation].every(Number.isFinite)||!own(zones,p.zone))return '정원 안에서 자리를 골라 주세요.';
  ...
  if(Math.hypot(p.x/25.3,p.z/20.9)+r/20.9>.9||Math.hypot((p.x-12)/3,(p.z+3)/5)<1+r/3)return '물가에서 조금 떨어진 풀밭에 놓아 주세요.';
