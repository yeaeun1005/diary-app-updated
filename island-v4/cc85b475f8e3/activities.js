  ...
  v4h('div',{className:'v4-activity-columns'},
   ...
   v4h('div',{className:'v4-work-card'},
    ...
    v4h(React.Fragment,null,
     ...
     draft.step===1&&v4h(React.Fragment,null,v4h('span',{className:'v4-eyebrow'},'02 · 내 마음의 바람'),v4h('h4',null,fiction?'이 인물에게 무엇이 중요했을까요?':'나에게 무엇이 중요했나요?'),v4h('p',null,'원했던 것이나 필요했던 도움을 떠올려 봐요.'),field('need','중요했던 것 또는 필요했던 도움','함께하고 싶었어. 잠깐 쉬고 싶었어…'),v4h('div',{className:'v4-chips'},['이해받고 싶었어요','함께하고 싶었어요','편안히 쉬고 싶었어요','아직 모르겠어요'].map(w=>btn(w,()=>update('need',w))))),
     draft.step===2&&v4h(React.Fragment,null,v4h('span',{className:'v4-eyebrow'},'03 · 작은 다음 걸음'),v4h('h4',null,'나에게 도움이 될 것을 찾아봐요'),v4h('p',null,'모든 칸을 채울 필요는 없어요. 바로 행동하지 않고 더 생각해도 좋아요.'),field('action',fiction?'이야기 속에서 한 행동 (선택)':'그때 내가 한 행동 (선택)','기억나는 만큼만 적어요.',2),field('helpful','그 행동이 도움이 되었나요? (선택)','도움이 되었거나 아쉬웠던 점, 아직 모르는 점.',2),field('nextPlan','필요한 도움이나 해 볼 행동 (선택)','믿을 수 있는 사람에게 이야기하기, 잠시 쉬기…',2)),
...
function V4Archive({me,entries,data,change,onLeavePlace,onUpdate,onDelete,onLegacy}){
 ...
 function record(e){const w=V4ActivityModel.words(e,data.choices);return v4h('article',{className:'v4-journal-card',key:e.ts},v4h('header',null,v4h('span',{className:'v4-eyebrow'},e.date),v4h('span',{className:'v4-private-tag'},'나만의 일기')),
 ...
 return v4h(React.Fragment,null,v4h(V4ActivityHeading,{kind:'archive',kicker:'기록 보관함 · 나에게 남긴 이야기',title:'지난 마음을 다시 만나요.',copy:'달력 속 하루를 펼치거나, 여러 날의 기록을 함께 돌아봐요.'}),
  ...
  v4h('div',{className:'v4-archive-extra'},v4h('button',{className:'v4-text',onClick:onLegacy},'세부 분석·기록 도구')),
