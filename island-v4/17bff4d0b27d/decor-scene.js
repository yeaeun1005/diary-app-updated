...
function v4DecorFallback(id,item){
 ...
 if(id==='bench'){mesh(new T.BoxGeometry(1.85,.15,.78),'#c79b62',0,.55,0);mesh(new T.BoxGeometry(1.85,.58,.13),'#d8b17b',0,1.05,-.34);for(const x of[-.7,.7])mesh(new T.BoxGeometry(.14,.55,.65),'#b38a57',x,.25,0);}
  ...
  for(const z of[-.45,.45])for(const side of[-1,1]){const leg=mesh(new T.BoxGeometry(.13,2,.13),'#bd945f',side*.80,1,z);leg.rotation.z=side*.17;}
  mesh(new T.BoxGeometry(2.25,.17,.15),'#d1a675',0,1.92,0);mesh(new T.BoxGeometry(.92,.15,.62),'#efd188',0,.68,0);
  for(const x of[-.4,.4])mesh(new T.CylinderGeometry(.025,.025,1.18,6),'#e7d4a6',x,1.31,0);
 }
 else if(id==='lighthouse'){
  for(let i=0;i<3;i++)mesh(new T.CylinderGeometry(.50-i*.07,.57-i*.07,.52,12),i===1?'#e9a397':'#f1e2bd',0,.26+i*.52,0);
  mesh(new T.CylinderGeometry(.41,.41,.43,12),'#ffe0a0',0,1.77,0);mesh(new T.ConeGeometry(.69,.40,12),'#76aaa0',0,2.18,0);
  mesh(new T.CylinderGeometry(.61,.69,.12,12),'#d3c6ac',0,.06,0);
 }
 else if(id==='flowerBoat'){
  const boat=mesh(new T.SphereGeometry(1,16,8),'#c89665',0,.33,0);boat.scale.set(1.04,.32,.48);
  mesh(new T.BoxGeometry(1.64,.12,.62),'#976c49',0,.54,0);
  for(let i=0;i<3;i++){const flower=v4BuildReward(T,'flower');flower.scale.setScalar(.68);flower.position.set((i-1)*.49,.52,0);g.add(flower);}
 }
...
function v4DecorObject(id,item){
 ...
 const modelId=item.modelId||id,fallback=v4DecorFallback(modelId,item);
 root.add(fallback);root.userData.asset=item.reward?'built-in':'loading';
 if(!item.reward)V4Assets.load('decor-'+modelId).then(template=>{if(dead)return;const model=V4Assets.instance(template,item);root.remove(fallback);v4DisposeDecor(fallback);root.add(model);root.userData.asset='ready';}).catch(()=>{root.userData.asset='fallback';});
