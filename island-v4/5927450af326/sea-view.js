...
function V4SeaView({ me, entries, reward, arrive, onBack, onGo, onTour, tutOn }) {
  ...
  useEffect(function () {
    ...
    var mine = V4_SEA_BEFORE?buildIsland(seed):v4SeaOwnIsland(reward);
    var myHolder = new THREE.Group();
    myHolder.add(mine);
    if(V4_SEA_BEFORE)myHolder.add(buildVegMeshes(vegCached(mine, islandState(entries), { seed: seed })));
    ...
    function ensureFull(e, det) {
      ...
      finishFull(e, g, vegCached(g, e.state, { seed: e.seed }), det, false);   // 캐시(2026-09-15): 두 번째 진입은 배치를 건너뛴다
    }
    ...
    function buildStep(deadline) {
      while (buildQ.length && performance.now() < deadline) {
        ...
        if (!job.g) { job.g = makeIsland(e.seed, { detail: det, tone: seaGrp ? seaGrp.userData.tone : 0.5 }); job.key = vegKey(job.g, e.state, { seed: e.seed }); continue; }
        var items = vegGet(job.key);
        if (!items) {
    ...
    function sailTo(e) {
      ...
      setSel(null);
      setSailing({ label: e.home ? "내 섬" : e.name + "의 섬" });
  ...
  if(!V4_SEA_BEFORE)return v4h('main',{className:'v4-sea','aria-label':'우리 반 바다'},
    ...
    v4h('header',{className:'v4-sea-header'},v4h('div',{className:'v4-sea-title'},v4h('span',null,'마음 바다 탐험대'),v4h('h1',null,'우리 반 바다'),v4h('p',null,'서로의 섬에, 마음 한 걸음')),v4h('button',{className:'v4-sea-home',disabled:!!sailing,onClick:()=>api.current.homeSail?api.current.homeSail():onBack(null)},'⌂ 내 섬으로')),
    v4h('section',{className:'v4-sea-finder','aria-label':'친구 섬 찾기'},v4h('div',{className:'v4-sea-search'},v4h('input',{'aria-label':'친구 이름으로 찾기',value:q,placeholder:'친구 이름으로 찾기',onChange:e=>{setQ(e.target.value);setListOpen(false);},onKeyDown:e=>{if(e.key==='Enter'&&matches.length)pick(matches[0]);if(e.key==='Escape')setQ('');}}),v4h('button',{'aria-expanded':listOpen,onClick:()=>{setListOpen(!listOpen);setQ('');}},'친구 목록')),
      (listOpen||q.trim())&&v4h('div',{className:'v4-sea-results'},(listOpen?names:matches).length?(listOpen?names:matches).map(n=>v4h('button',{key:n.id,onClick:()=>pick(n)},v4h('span',null,'⚑'),n.name+'의 섬',v4h('span',null,'→'))):v4h('p',null,loading||(q?'이름을 다시 확인해 주세요.':'아직 친구 섬이 없어요.'))),
      todayIsle&&!q&&!listOpen&&!sel&&!sailing&&v4h('button',{className:'v4-sea-today','data-tut':'sea-today',onClick:()=>pick(todayIsle)},'✧ 오늘은 '+todayIsle.name+'의 섬에 가 볼까요?')),
    v4h('nav',{className:'v4-sea-tools','aria-label':'바다 화면 조작'},v4h('button',{onClick:()=>api.current.overview?.()},'전체 보기'),v4h('button',{'aria-label':'바다 축소',onClick:()=>api.current.zoom?.(1.22)},'−'),v4h('button',{'aria-label':'바다 확대',onClick:()=>api.current.zoom?.(.82)},'+'),onTour&&v4h('button',{'aria-label':'바다 둘러보기',onClick:()=>onTour(()=>{try{localStorage.setItem('tutSea','1');}catch{}setTutSeen(true);})},'?'),v4h(SndBtn,{small:true})),
    !sel&&!sailing&&v4h('p',{className:'v4-sea-hint','data-tut':'sea-hint'},'섬을 눌러 가까이 살펴보세요',v4h('small',null,'끌어서 둘러보기 · 두 번 누르면 바로 출항')),
    sel&&!sailing&&v4h('section',{className:'v4-sea-destination','aria-label':'선택한 섬'},v4h('div',null,v4h('small',null,'이번에 가 볼 곳'),v4h('h2',null,sel.home?'내 마음섬':sel.name+'의 섬')),v4h('button',{className:'v4-sea-sail',onClick:()=>sel.home?api.current.homeSail?.():api.current.sail?.(sel.id)},'배 타고 가기 →'),v4h('button',{'aria-label':'섬 선택 닫기',onClick:()=>api.current.clear?.()},'×')),
    (loading||sailing||err)&&v4h('p',{className:'v4-sea-status',role:err?'alert':'status'},err||loading||(sailing.label+'으로 가는 중…')),
  ...
  return React.createElement("div", { style: { position: "relative", width: "100%", height: "100%", overflow: "hidden", background: "#dfeef2" } },
    ...
    sel && !sailing ? React.createElement("div", { className: "tut-dim",
      ...
      React.createElement("b", { style: { color: "#0e7490" } }, sel.home ? "내 섬" : sel.name + "의 섬"),
      React.createElement("button", {
    ...
    }, React.createElement("b", { style: { color: "#0e7490" } }, sailing.label), "으로 가는 중…") : null);   // ⑥-1의 "섬 구경은 아직 준비 중이에요" 도착 팝업은 ⑥-2 뒤로 닿을 수 없어 지웠다(2026-09-15 저녁)
}
