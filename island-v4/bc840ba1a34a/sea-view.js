...
function V4SeaView({ me, entries, reward, arrive, onBack, onGo, onTour, tutOn }) {
  ...
  if(!V4_SEA_BEFORE)return v4h('main',{className:'v4-sea','aria-label':'우리 반 바다'},
    ...
    v4h('header',{className:'v4-sea-header'},v4h('div',{className:'v4-sea-title'},v4h('h1',null,v4h('img',{src:new URL('sea-title.svg',V4_UI_ASSETS).href,alt:'마음 바다 탐험대 · 우리 반 바다'})),v4h('p',null,'서로의 섬에, 마음 한 걸음')),v4h('button',{className:'v4-sea-home',disabled:!!sailing,onClick:()=>api.current.homeSail?api.current.homeSail():onBack(null)},'⌂ 내 섬으로')),
    ...
    v4h('nav',{className:'v4-sea-tools','aria-label':'바다 화면 조작'},v4h('button',{onClick:()=>api.current.overview?.()},'전체 보기'),v4h('button',{'aria-label':'바다 축소',onClick:()=>api.current.zoom?.(1.22)},v4ToolIcon('minus')),v4h('button',{'aria-label':'바다 확대',onClick:()=>api.current.zoom?.(.82)},v4ToolIcon('plus')),onTour&&v4h('button',{'aria-label':'바다 둘러보기',onClick:()=>onTour(()=>{try{localStorage.setItem('tutSea','1');}catch{}setTutSeen(true);})},'?'),v4h(V4SoundButton)),
