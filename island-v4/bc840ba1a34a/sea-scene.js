...
function v4SeaTag(name,mine){
 ...
 const text=mine?'내 마음섬':name+'의 섬';let size=48;c.font='700 '+size+'px Maplestory,sans-serif';while(c.measureText(text).width>552&&size>20)c.font='700 '+(--size)+'px Maplestory,sans-serif';c.fillStyle=mine?'#426c58':'#635e4b';c.textAlign='center';c.textBaseline='middle';c.fillText(text,320,78,568);
...
function v4SeaOwnIsland(reward,options={}){
 ...
 const land=new T.PlaneGeometry(54*S,46*S,options.miniature?24:54,options.miniature?22:46);land.rotateX(-Math.PI/2);const p=land.attributes.position,colors=[];
