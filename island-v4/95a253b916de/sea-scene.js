...
function v4SeaTag(name,mine){
 ...
 const text=mine?'내 마음섬':name+'의 섬';let size=48;c.font='700 '+size+'px system-ui';while(c.measureText(text).width>552&&size>20)c.font='700 '+(--size)+'px system-ui';c.fillStyle=mine?'#426c58':'#635e4b';c.textAlign='center';c.textBaseline='middle';c.fillText(text,320,78,568);
 const tx=new T.CanvasTexture(canvas);tx.encoding=T.sRGBEncoding;tx.userData.v4SeaOwned=true;
 const tag=new T.Mesh(new T.PlaneGeometry(5.3,1.4),new T.MeshBasicMaterial({map:tx,transparent:true,depthWrite:false,depthTest:false,toneMapped:false}));tag.rotation.set(-.545,Math.PI/4,0,'YXZ');tag.position.y=mine?4.4:TIER_Y[2]+3.4;tag.renderOrder=900;tag.frustumCulled=false;return tag;
}
...
function v4SeaFriendIsland(seed,options){
 const island=buildIsland(seed,options);
 // Local material clones created by buildIsland; original topology/plant placements untouched.
 island.traverse(o=>{if(!o.isMesh)return;for(const m of Array.isArray(o.material)?o.material:[o.material]){
  if(!m.userData.su)continue;
  const kind=m.customProgramCacheKey?.();
  if(kind==='sea-shore'){m.opacity=.30;m.userData.deepBase=false;m.userData.su.uShallow.value.set('#8ec8bc');m.userData.su.uShore.value.set('#b3d8c6');m.onBeforeCompile=shader=>{shader.vertexShader='attribute float aT;varying float v4Shore;\n'+shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nv4Shore=aT;');shader.fragmentShader='varying float v4Shore;\n'+shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.a*=1.-smoothstep(.1,1.,v4Shore);');};m.color.set('#b1d7c3');m.customProgramCacheKey=()=> 'v4-soft-shore';}
  else if(kind==='sea-foam')m.opacity*=.40;
  else if(kind==='sea-wet')m.opacity=.4;
 }});return island;
}
function v4SeaBoat(){
...
function v4SeaOwnIsland(reward){
 ...
 const land=new T.PlaneGeometry(54*S,46*S,54,46);land.rotateX(-Math.PI/2);const p=land.attributes.position,colors=[];
 ...
 for(let i=0;i<60;i++){const a=i*Math.PI*2/60,x=Math.cos(a)*22.1,z=Math.sin(a)*18.2;if(v4Dock(x,z))continue;put('#b9baa4',x,.15,z,.75,.6,.7);}
 ...
 function attach(id,fallback,transform){V4Assets.load(id).then(t=>{if(g.userData.v4SeaDead)return;const model=V4Assets.instance(t,transform);model.position.multiplyScalar(S);model.scale.setScalar(S);g.add(model);fallback.visible=false;}).catch(()=>{});}
 ...
 const boat=v4SeaBoat();boat.position.set(9.3*S,.10,21*S);g.add(boat);g.userData={...g.userData,boat,boatDir:Math.PI/2-.2,dockTh:Math.PI/2-.2,r:6.5,coast:()=>6.5};return g;
}
