...
const V4Sharing=(()=>{
 ...
 async function records(who){const raw=await sGetStrict(empKey(id(who)));return Object.entries(raw||{}).map(([key,e])=>{e=e||{};const parts=key.split('_');return{key,name:e.n||'',q:e.q||'LA',l:e.l||'',ts:e.ts||0,from:parts[1]||'',date:parts[0]||'',message:typeof e.m==='string'?e.m.slice(0,100):''};}).sort((a,b)=>a.ts-b.ts);}
 ...
 return{entryId,same,decode,list,records,publish,write,clear,cleanMessage,send};
})();
...
function V4SharedDiaryPanel({me,friend,onClose}){
 ...
 const[list,setList]=useState([]),[ready,setReady]=useState(false),[error,setError]=useState(''),[records,setRecords]=useState(null),[mine,setMine]=useState(null),[socialError,setSocialError]=useState(false),[retry,setRetry]=useState(0);
 useEffect(()=>{if(own)return;let active=true;setSocialError(false);Promise.all([V4Sharing.records(friend.id),isleLoad(me.id)]).then(([r,m])=>{if(active){setRecords(r);setMine(m.state);}}).catch(()=>{if(active)setSocialError(true);});return()=>{active=false;};},[me.id,friend.id,retry]);
 ...
 return v4h(V4Dialog,{title:own?'내가 공개한 일기':friend.name+'의 공개 일기',onClose},socialError&&v4h('p',{role:'alert'},'공감 상태를 불러오지 못했어요. ',v4h('button',{onClick:()=>setRetry(n=>n+1)},'공감 다시 불러오기')),v4h('p',null,own?'내가 우리 반 친구에게 공개한 글이에요. 일기 보관함에서 언제든 비공개로 바꿀 수 있어요.':'친구가 일기마다 직접 선택해 공개한 글만 보여요. 기본은 비공개이고, 언제든 공개를 취소할 수 있어요.'),error?v4h('p',{role:'alert'},error):!ready?v4h('p',{role:'status'},own?'내가 공개한 글을 찾고 있어요…':'친구가 공개한 글을 찾고 있어요…'):list.length?list.map(e=>v4h('article',{className:'v4-shared-entry',key:e.key},v4h('small',null,e.date),v4h('p',{className:'v4-journal-text'},e.text),v4h('div',{className:'v4-record-words'},e.emotions.filter(w=>own||friend.state.some(s=>s.label===w)).map(w=>v4h('span',{key:w},w))),!own&&v4h(V4DiaryEmpathy,{me,friend,entry:e,records,mine,onRecords:setRecords}))):v4h('div',{className:'v4-shared-empty'},v4h('span',{'aria-hidden':true},'✉'),v4h('h3',null,'아직 공개한 일기가 없어요'),v4h('p',null,own?'일기 보관함에서 공개할 일기를 직접 고를 수 있어요.':'친구가 아직 일기를 공개하지 않았어요. 일기가 있어도 공개를 선택하지 않으면 여기에 나오지 않아요.')));
}
function V4ReceivedMessages({me}){
 const[notes,setNotes]=useState([]),[error,setError]=useState('');useEffect(()=>{let alive=true;V4Sharing.records(me.id).then(rows=>{if(alive)setNotes(rows.filter(e=>e.message).reverse());}).catch(()=>{if(alive)setError('응원 편지를 불러오지 못했어요.');});return()=>{alive=false;};},[me.id]);
