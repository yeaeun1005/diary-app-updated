...
function V4CareCard({student,record,ready,busy,error,onSave}){
 ...
 return v4h('article',{className:'v4-care-card','data-care-id':String(student.id)},
  ...
  v4h('section',{className:'v4-care-conversation'},v4h('h4',null,'이렇게 이야기를 시작해 보세요'),v4h('p',null,'요즘 학교에서 지내면서 마음에 남은 일이 있니? 선생님이 들어 줬으면 하는 이야기가 있을까?'),student.topLab?.length>0&&v4h('p',null,'일기에 '+student.topLab.slice(0,2).map(x=>x.label).join(', ')+'이라는 말을 썼구나. 그때 어떤 일이 있었는지 말해 줄래?'),v4h('small',null,'조용한 자리에서 묻고, 아이가 말하고 싶을 때까지 기다려 주세요. 감정 분석만으로 이유를 단정하지 않아요.')),
