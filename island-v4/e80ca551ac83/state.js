...
const V4State=(()=>{
 ...
 function reduce(input,event,now=Date.now()){
  ...
  const daily=typeof V4Daily==='undefined'?require('./exploration-upgrade/daily-model.js'):V4Daily;
  switch(event.type){
   case 'dailyWrite':case 'dailyAnalysis':case 'dailyInsight':case 'dailyTalk':daily.apply(s,event,now);break;
   ...
   case 'forget':for(const [day,d] of Object.entries(s.daily||{}))if(d.entryTs===event.ts)delete s.daily[day];if(s.context?.kind==='personal'&&s.context.entryTs===event.ts)s.context=null;delete s.choices[String(event.ts)];delete s.talk[String(event.ts)];if(s.selectedEntryTs===event.ts)s.selectedEntryTs=null;if(q.source?.kind==='personal'&&q.source.entryTs===event.ts){q.source=null;q.response=null;q.feedbackSeen=false;q.stage=q.completedAt?'complete':'write';}s.draft='';break;
   case 'edit':daily.invalidate(s,event.ts);if(q.source?.kind==='personal'&&q.source.entryTs===event.ts){q.response=null;q.feedbackSeen=false;q.stage='compare';}break;
