...
function V4AppInner(props){
 ...
 function earned(){setCelebrate(true);setPanel('complete');}
 ...
 async function change(event){if(event.type.startsWith('daily'))event={...event,mode:await V4RewardSettings.forStudent(me)};const before=V4Daily.current(V4State.load(me)).completedAt;const next=await V4State.dispatch(me,event);if(alive.current){setData(next);if(!before&&V4Daily.current(next).completedAt)earned();}return next;}
 ...
 return v4h(React.Fragment,null,
  view==='friend'?v4h(V4FriendWorld,{key:friend.id,state:friend.state,myState:state,me,friend,panel,onOpen:open,onHome}):v4h(V4World,{me,data,change,panel,onOpen:open,onLogout,onHome,placeRequest,editDecor,onDecorClose:()=>setEditDecor(null),onDecorLibrary:()=>{setEditDecor(null);setPanel('reward');}}),
  ...
  panel==='complete'&&v4h(V4Dialog,{title:'오늘의 마음 탐험 완료!',onClose:close},v4h('section',{className:'v4-completion',role:'status'},v4h('div',{className:'v4-completion-stars','aria-hidden':true},'✦  ★  ✦'),v4h('span',{className:'v4-completion-medal','aria-hidden':true},'✓'),v4h('h3',null,'오늘 내 마음을 조금 더 알게 되었어요!'),v4h('div',{className:'v4-completion-steps'},[['감정일기쓰기','완료'],['마음 분석하기',V4Daily.current(data).analysisCheckedAt?'완료':'선택 활동'],['마음대화하기',V4Daily.current(data).talkStatus==='done'?'완료':V4Daily.current(data).talkStatus==='skipped'?'오늘은 대화할 내용 없음':'선택 활동']].map(([s,status])=>v4h('span',{key:s},s,v4h('small',null,status)))),v4h('button',{className:'v4-primary',onClick:()=>setPanel('reward')},data.reward.item?'나의 선물 보러 가기':'완료 선물 고르기'),v4h('button',{className:'v4-text',onClick:close},'섬에서 자유롭게 놀기'))),
...
function V4World({me,data,change,panel,onOpen,onLogout,onHome,placeRequest,editDecor,onDecorClose,onDecorLibrary}){
 ...
 function tutorialStage(id){setTourStage(id);api.current?.stop();api.current?.setTourFocus(id==='write'?'write':id==='dock'?'dock':null);api.current?.setDestination(id==='write'?'write':id==='dock'?'dock':null);}
 ...
 useEffect(()=>{api.current?.pause(!!panel||help||!!editDecor||tourWelcome||(tour&&tourStage!=='walk'));},[panel,help,quality,editDecor,tourWelcome,tour,tourStage]);useEffect(()=>{api.current?.setDecoration(data.reward,data.decor);},[data.reward.item,data.reward.spot,data.decor,quality]);
 ...
 return v4h('main',{className:'v2-world v4-world'+(tour?' is-touring':'')+(editDecor?' is-decorating':''),'aria-hidden':panel?true:undefined,inert:panel?'':undefined},v4h('div',{className:'v2-canvas',ref:host,'data-testid':'v4-world'}),v4h('div',{className:'v2-vignette'}),v4h('div',{className:'v2-passage-wash'}),
  ...
  ui.pin&&!ui.pin.near&&!panel&&v4h('div',{className:'v4-beacon-label','data-world-anchor':'destination'},v4h('span',{'aria-hidden':true},'✦'),v4h('strong',null,ui.pin.title),v4h('small',null,ui.pin.id==='decoration'?'내 섬에 놓았어요':ui.pin.near?'E를 눌러 열어요':'이곳으로 걸어와요')),
  ...
  help&&v4h(V4Dialog,{title:'편안하게 탐험하기',onClose:()=>setHelp(false)},v4h('p',null,'방향키 또는 WASD로 걸어요. 길을 눌러 걸을 수도 있어요. 드래그나 마우스 휠로 둘러봐요.'),v4h('p',null,'장소에 다가가면 E 표시가 떠요. E 키나 표시를 누르면 열려요.'),v4h('div',{className:'v4-actions'},button('표준 화질',()=>setQuality('standard')),button('가벼운 화질',()=>setQuality('low'))),v4h('p',{className:'v4-muted'},MEM_ONLY?'체험 중 · 새로고침하면 처음으로 돌아가요.':'로그인한 계정에 기록과 탐험 진행을 저장해요.'),button('섬 사용법 다시 보기',startTutorial,'v4-primary'),button('처음으로',onLogout)),
  ...
  tour&&v4h(React.Fragment,null,['walk','write','dock'].map(id=>v4h('div',{key:id,className:'v4-tutorial-anchor','data-tut':'v4-'+id,'aria-hidden':true})),v4h(V4IslandTutorial,{api,onStage:tutorialStage,onDone:endTutorial})),
...
function V4Activities({me,data,change,entries,panel,onLeavePlace,onEarned,onClose,onAdd,onUpdate,onDelete,setEntries,onLegacy}){
 ...
 async function saveSource(quest){await run(async()=>{
  {
   ...
   const progress=await change({type:'dailyWrite',ts:entry.ts,date:entry.date});
   await change({type:'selectEntry',ts:entry.ts});await change({type:'draft',text:''});
   if(V4Daily.current(progress).completedAt&&!V4Daily.current(data).completedAt){pendingEntry.current=null;setText('');V4WritingDrafts.delete(helperKey);setWriting({emotions:[],situations:[],date:dayStamp()});return;}
   pendingEntry.current=null;setText('');V4WritingDrafts.delete(helperKey);setWriting({emotions:[],situations:[],date:dayStamp()});
  }
  setNotice('저장했어요.');onLeavePlace('review','일기를 저장했어요. 마음을 살펴보고 싶다면 빛나는 관찰대로 가세요.');
 });}
 ...
 return v4h(V4Dialog,{title:V4_TABS.find(t=>t[0]===panel)?.[1]||'마음 활동',onClose,wide:true,activity:panel},v4h('p',{className:'v4-place-caption'},({write:'나무 책상',review:'물가 관찰대 · 마음 말의 뜻과 차이를 살펴보는 곳',talk:'마음 우체통 · 마음에 남은 일을 이야기하는 곳',practice:'작은 작업대 · 천천히 연습하는 곳',archive:'기록 보관함 · 나의 일기를 다시 만나는 곳'})[panel]),v4h('div',{className:'v4-activity-body'},
  panel==='write'&&v4h(React.Fragment,null,v4h('div',{className:'v4-section-head'},v4h('small',null,q.active?'탐험 01 · 이야기에서 시작해요':'나만의 기록'),v4h('h3',null,'오늘 기억에 남는 순간이 있나요?'),v4h('p',null,'그때 어떤 마음이 들었나요? 특별한 일이 아니어도 좋아요.')),
   v4h(React.Fragment,null,v4h('div',{className:'v4-write-date'},v4h('label',null,'일기 날짜 ',v4h('input',{type:'date',value:writing.date,onInput:e=>updateWriting({date:e.target.value}),onChange:e=>updateWriting({date:e.target.value}),disabled:busy})),b('오늘',()=>updateWriting({date:dayStamp()}))),v4h('label',{className:'v4-label',htmlFor:'v4-diary'},'내 이야기'),v4h('textarea',{id:'v4-diary',value:text,disabled:busy,onChange:e=>{setText(e.target.value);change({type:'draft',text:e.target.value}).catch(()=>setNotice('임시 글을 보관하지 못했어요.'));},placeholder:'어떤 상황에서 어떤 감정을 느꼈나요?',rows:5}),b(help?'감정·상황 도움 접기':'감정·상황 도움받기',()=>setHelp(!help),'v4-text'),help&&v4h(V4WritingHelper,{text,emotions:writing.emotions,situations:writing.situations,onEmotions:emotions=>updateWriting({emotions}),onSituations:situations=>updateWriting({situations}),disabled:busy})),
   v4h('div',{className:'v4-actions v4-write-submit'},b(busy?'저장 중…':'일기 저장하기',()=>saveSource(q.active),'v4-primary',(V4ActivityModel.dateNumber(writing.date)===null||!V4StudentTools.compose(text,writing.emotions,writing.situations).text)),b('나중에 이어쓰기',onClose))),
