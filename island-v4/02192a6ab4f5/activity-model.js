...
const V4ActivityModel=(()=>{
 ...
 function words(entry,choices){
  return {auto:unique((entry.analysis?.hits||[]).filter(h=>h.token!=='직접선택').map(h=>h.label)),chosen:unique([...(entry.analysis?.studentSelection||[]),...(entry.analysis?.hits||[]).filter(h=>h.token==='직접선택').map(h=>h.label),...(choices?.[String(entry.ts)]||[])])};
 }
