...
function v4CreateFriendWorld(host,state,callbacks,saved,options={}){
 ...
 function outlineObject(id,source){if(outlines[id])scene.remove(outlines[id]);const edge=source.clone(true);edge.traverse(o=>{if(o.isMesh){o.material=outlineMat;o.castShadow=o.receiveShadow=false;o.renderOrder=2;}});edge.visible=false;scene.add(edge);outlines[id]=edge;}
  ...
  function tree(x,z,size=1,kind='oak',record){const y=v2Height(x,z);if(v2Radius(x,z)>.88)return;colliders.push({x,z,r:.36*size,kind:'tree'});const palette=record?(kind==='willow'?['#86a97c','#739875','#9db887']:['#7aa163','#a1b975','#678c60']):x< -10&&z<0?['#dca6b7','#e9bbc5','#c493ac']:x<0&&z>8?['#b5a4c6','#c5b7d3','#a998b9']:['#88af85','#b1c993','#7caa91'];
  ...
  function trialBoard(){
    ...
    V2BoardAsset.load().then(template=>{
      ...
      scene.add(model);outlineObject('board',model);fallback.visible=false;boardTrial.loaded=1;boardTrial.state='ready';boardTrial.loadMs=performance.now()-started;
  ...
  let dockArmed=Math.hypot(actor.position.x-V2.stops.dock.x,actor.position.z-V2.stops.dock.z)>4.6;
  ...
  function interact(action){if(hero||paused||passages.busy)return;if(action==='sea'&&(!dockArmed||nearest?.action!=='sea'))return;clear();if(isGuest&&action==='diary'){callbacks.open('shared');return;}passages.begin(action);}
  ...
  function wheel(e){if(paused||passages.busy)return;e.preventDefault();const delta=(Math.abs(e.deltaX)>Math.abs(e.deltaY)?e.deltaX:e.deltaY)*(e.deltaMode===1?16:e.deltaMode===2?host.clientHeight:1);yaw+=Math.max(-240,Math.min(240,delta))*.003;}
  ...
  function tick(){if(disposed)return;raf=requestAnimationFrame(tick);if(hero){const now=performance.now();if(now-heroFrame<32)return;heroFrame=now;}const rawDt=clock.getDelta(),dt=document.hidden?0:Math.min(rawDt,.04),t=clock.elapsedTime;if(!paused&&rawDt<.25){frameSum+=rawDt*1000;frameN++;}let vx=0,vz=0,moving=false;
    ...
    function anchor(selector,x,y,z){const el=host.parentElement.querySelector(selector);if(!el)return;const p=new T.Vector3(x,y,z).project(cam);el.style.transform='translate3d('+((p.x*.5+.5)*host.clientWidth)+'px,'+((-p.y*.5+.5)*host.clientHeight)+'px,0) translate(-50%,-100%)';el.style.visibility=p.z>-1&&p.z<1&&Math.abs(p.x)<1&&Math.abs(p.y)<1?'visible':'hidden';}
    anchor('[data-friend-anchor=character]',actor.position.x,actor.position.y+3.0,actor.position.z);
    if(destinationPoint)anchor('[data-friend-anchor=destination]',destinationPoint.x,v2WalkY(destinationPoint.x,destinationPoint.z)+2.8,destinationPoint.z);
    for(const[id,o]of Object.entries(outlines))o.visible=!paused&&!passages.busy&&nearest?.action===(id==='dock'?'sea':'board');
    ren.render(scene,cam);
    if(t-lastUI>.10){lastUI=t;if(Math.hypot(actor.position.x-V2.stops.dock.x,actor.position.z-V2.stops.dock.z)>4.6)dockArmed=true;const previous=nearest;nearest=null;let best=Infinity;for(const s of Object.values(V2.stops)){if(!s.action||(s.action==='sea'&&!dockArmed))continue;const d=Math.hypot(actor.position.x-s.x,actor.position.z-s.z),radius=s.action==='sea'?2.1:3.1;if(d<radius&&d<best){best=d;nearest=s;}}if(previous&&(previous.action!=='sea'||dockArmed)){const d=Math.hypot(actor.position.x-previous.x,actor.position.z-previous.z),radius=previous.action==='sea'?2.55:3.55;if(d<radius&&(!nearest||nearest===previous||best>d-.55))nearest=previous;}
      const zoneNear=V2.zones.reduce((a,b)=>Math.hypot(actor.position.x-a.x,actor.position.z-a.z)<Math.hypot(actor.position.x-b.x,actor.position.z-b.z)?a:b);const zone=actor.position.z>19?{name:'바람 부두',sub:isGuest?'우리 반 바다로 가는 배가 기다려요':'바다 건너, 친구의 마음 섬으로'}:v2Radius(actor.position.x,actor.position.z)>.76?{name:'모래 해안',sub:'물결을 따라 천천히 걸어요'}:Math.hypot(actor.position.x+5,actor.position.z-7)<10?{name:'마음 마당',sub:'나의 이야기가 시작되는 곳'}:{...zoneNear,name:zoneNear.name+' 숲길'};const nearGrowth=growth.find(g=>Math.hypot(g.x-actor.position.x,g.z-actor.position.z)<1.85);
