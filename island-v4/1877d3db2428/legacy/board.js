...
function V2Board({ me, myState, arrive, backLabel, onBack, onSeen, onTour, tutOn }) {
  ...
  const [customOn,setCustomOn]=useState(false),[custom,setCustom]=useState("");
  ...
  function toggleChip(c) {
    ...
    setForm(function (f) {
      ...
      if (f.fl.length + (customOn ? 1 : 0) >= 5) { setWarn("감정은 기타를 포함해 최대 5개까지 골라요."); return f; }
      return { ...f, fl: f.fl.concat([{ l: c.l, q: c.q }]) };
    });
  }
  function startWrite() {
    var extra=(ans?.fl||[]).find(x=>!chips.some(c=>c.l===x.l));setCustomOn(!!extra);setCustom(extra?.l||"");
    setForm(ans ? { o: plazaAnsText(ans), f: "", w: "", fl: ans.fl.filter(x=>x!==extra) } : { o: "", f: "", w: "", fl: [] });   // 옛 세 칸 답은 한 글로 이어서 고친다(2026-09-16)
  ...
  function fixName(nm) {
    ...
    setCustom(value=>plazaNameFix(value,nm));setWarn("");
  }
  async function save() {
    if (saving) return;
    var o = form.o.trim(), f = "", w = "";   // 2026-09-16: 한 칸. 마음 말 칩은 골라도 안 골라도 된다
    if (!o) { setWarn("답을 먼저 써 주세요."); return; }
    var label=plazaCleanText(custom).slice(0,40),fl=form.fl.slice();
    if(customOn&&!label){setWarn("기타 감정을 적어 주세요.");return;}
    if(customOn&&!fl.some(x=>x.l===label)){var known=chips.find(c=>c.l===label);fl.push({l:label,q:known?.q||""});}
    if(fl.length>5){setWarn("감정은 기타를 포함해 최대 5개까지 골라요.");return;}
    var names=[...roster,me.name].filter(Boolean),hit=plazaNameHit([o,...fl.map(x=>x.l)].join(" "),names,"")||names.find(nm=>fl.some(x=>x.l===nm));
    var flagged = false;
    if (hit) {
      // 첫 번째는 막고 바꾸기를 돕는다. 두 번째는 저장하되 숨김 — 정말 그 말을 쓰고 싶은 아이는 쓸 수 있고,
      // 광장에는 교사 확인 없이 절대 나가지 않는다.
      if (nameBlock < 1) { setNameBlock(1); setWarn({ text: "'" + hit + "'은 이름이에요. 나와 친구의 이름은 쓰지 않아요.", name: hit }); return; }
      flagged = true;
    }
    setSaving(true);
    try {
      await plazaAnswerPut(code, q.id, me, { o: o, f: f, w: w, fl: fl, flagged: flagged });
      setAns({ sid: String(me.id), o: o, f: f, w: w, fl: fl.slice(), ts: nowTs(), s: flagged ? 2 : 1, nmf: flagged });
  ...
  if (loading) body = React.createElement("div", { style: { fontSize: 14, color: "#5b7684" } }, "오늘의 질문을 가져오는 중…");
    ...
    React.createElement("div", { "data-tut":"plz-q", style: { fontFamily: "'Jua',sans-serif", fontSize: qFont, color: "#0e7490", lineHeight: 1.4, marginBottom: 6 } }, q.t),
    q.nm ? React.createElement("div", { style: { fontSize: 12, color: "#9a5b2b", background: "#fff4e6", borderRadius: 8, padding: "5px 10px", marginBottom: 6 } }, "저장하면 이름 없이 자동 공개돼요. 나와 친구의 이름은 쓰지 않아요.") : null,
    // 2026-09-16 개편: 관찰·느낌·바람 세 칸 → 답 한 칸(자유롭게) + 마음 말 칩(선택). 이름 검사·교사 승인은 그대로
    React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "#0e7490", marginTop: 6 } }, "여기에 내 답과 그때의 마음을 써요"),
    React.createElement("textarea", { "data-tut":"plz-answer", "aria-label":"내 답 쓰기", value: form.o, maxLength: PLAZA_MAX, rows: 4, placeholder: "생각나는 대로 써요. 나와 친구의 이름은 쓰지 않아요.",
    ...
    React.createElement("div", { "data-tut":"plz-emotions", style: { display: "flex", flexWrap: "wrap", gap: 5, marginTop: 5, maxHeight: 92, overflowY: "auto", padding: 2 } },
      chips.map(function (c) { return chipEl(c, form.fl.some(function (x) { return x.l === c.l; }), function () { toggleChip(c); }, 0.95); })),
    React.createElement("button",{type:"button","aria-pressed":customOn,onClick:()=>{if(!customOn&&form.fl.length>=5){setWarn("감정은 기타를 포함해 최대 5개까지 골라요.");return;}setCustomOn(!customOn);setWarn("");},style:{...bSoft,marginTop:8,minHeight:44}},"기타"),
    customOn&&React.createElement("label",{style:{display:"block",marginTop:8,fontWeight:700,fontSize:13}},"기타 감정 직접 쓰기",React.createElement("input",{type:"text",value:custom,maxLength:40,onChange:e=>{setCustom(e.target.value);setWarn("");},placeholder:"내가 느낀 감정을 적어 주세요.",style:{display:"block",boxSizing:"border-box",width:"100%",minHeight:44,marginTop:5,padding:"9px 12px",border:"1px solid #cfdde3",borderRadius:10,font:"inherit"}})),
    React.createElement("div", { style: { fontSize: 11, fontWeight: 600, color: "#6b8792", marginTop: 4 } }, "색깔이 없는 감정 단어는 내가 아직 사용해보지 않은 단어에요. 누르면 뜻을 알 수 있어요."),
    tipEl,
    ...
    React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 10 } },
      ...
      React.createElement("button", { "data-tut":"plz-save", onClick: save, disabled: saving, style: { ...bSoft, padding: "7px 18px", fontSize: 14, color: "#fff", background: "#0e7490", border: "1px solid #0e7490", opacity: saving ? 0.6 : 1 } }, saving ? "저장 중…" : "답 저장하기")));
    ...
    !ans ? React.createElement("div", null,
      React.createElement("div", { "data-tut": "plz-how", style: { display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" } },
        ...
        React.createElement("button", { onClick: startWrite, style: { ...bSoft, padding: "8px 20px", fontSize: 15, color: "#fff", background: "#0e7490", border: "1px solid #0e7490" } }, "내 답 쓰기")),
      // 쓰기 전에는 친구 답을 읽지 않는다(§8 "쓰기 전에 보면 베낀다"). 대신 몇 명이 답했는지만 알린다(게시판에도 같은 줄)
      React.createElement("div", { "data-tut":"plz-wait", style: { marginTop: 10, fontSize: 13, fontWeight: 600, color: "#5b7684", background: "#f4f8fa", borderRadius: 8, padding: "7px 10px", lineHeight: 1.5 } },
        "친구들의 답은 내 답을 쓴 뒤에 보여요.\n", nOthers ? "지금 " + nOthers + "명이 답했어요" : "아직 답한 친구가 없어요"))
  ...
  return React.createElement("div", {className:"v2-modal-shade", onPointerDown:function(e){e.stopPropagation();}},
    React.createElement("section", {className:"v2-board", role:"dialog", "aria-modal":true, "aria-label":"우리 반 게시판"},
      ...
      React.createElement("div", {className:"v2-board-body"}, !loading&&q&&React.createElement(V4ActivityGuide,{key:mode,me,kind:mode==="write"?"board-write":"board"}), err ? React.createElement("p", {role:"alert"}, err) : body),
