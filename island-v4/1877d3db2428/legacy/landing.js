...
function V2Landing({onLogin,onPrivacy,onCredits,authOpen,signedIn=false}){
  ...
  function login(e){opener.current=e.currentTarget;SND.play('start');onLogin();}
  ...
  return v2h('main',{className:'v2-landing v4-home v4-voyage-home','data-motion':reduced||authOpen||hidden?'paused':'playing',inert:authOpen?'':undefined},
    v2h('p',{className:'voyage-audience'},'초등 1-6학년, 국어, 도덕'),
    ...
    v2h('div',{className:'v2-home-actions'},v2h('div',{className:'voyage-cta-float'},
      ...
      v2h('button',{className:'v2-home-start',onClick:login,'aria-label':signedIn?'내 섬으로 이어가기':'로그인하고 시작하기'},
      ...
      v2h('span',{className:'voyage-cta-copy','aria-hidden':true},signedIn?v2h('span',{className:'voyage-resume-copy'},v2h('small',null,'내 섬으로'),'이어가기'):v2h(React.Fragment,null,word('cta-login-v5.svg','로그인하고','voyage-login-art'),word('cta-start-v5.svg','시작하기','voyage-start-art'))),v2h('span',{className:'v4-start-arrow','aria-hidden':true},arrow)))),
