...
function V4SharedDiaryPanel({me,friend,onClose}){
 ...
 return v4h(V4Dialog,{title:friend.name+'의 공개 일기',onClose},v4h('p',null,'친구가 일기마다 직접 선택해 공개한 글만 보여요. 기본은 비공개이고, 언제든 공개를 취소할 수 있어요.'),error?v4h('p',{role:'alert'},error):!ready?v4h('p',{role:'status'},'친구가 공개한 글을 찾고 있어요…'):list.length?list.map(e=>v4h('article',{className:'v4-shared-entry',key:e.key},v4h('small',null,e.date),v4h('p',{className:'v4-journal-text'},e.text),v4h('div',{className:'v4-record-words'},e.emotions.filter(w=>friend.state.some(s=>s.label===w)).map(w=>v4h('span',{key:w},w))))):v4h('div',{className:'v4-shared-empty'},v4h('span',{'aria-hidden':true},'✉'),v4h('h3',null,'아직 공개한 일기가 없어요'),v4h('p',null,'친구가 아직 일기를 공개하지 않았어요. 일기가 있어도 공개를 선택하지 않으면 여기에 나오지 않아요.')));
}
