...
function V4Talk({me,entries,data,change,onLeavePlace}){
 ...
 const seed=useMemo(()=>V4StudentTools.talkSeed(raw),[raw]);
 ...
 async function save(){
  if(lock.current||!any||(!fiction&&(!entry||load!=='ready')))return;lock.current=true;setBusy(true);setError('');
  try{
   ...
   v4h('div',{className:'v4-work-card v4-talk-work'},
    ...
    v4h(React.Fragment,null,
     v4h('section',{className:'v4-talk-question'},v4h('span',{className:'v4-eyebrow'},String(draft.step+1).padStart(2,'0')+' / 05 · '+current.label),v4h('h4',{ref:question,tabIndex:-1},current.question),
      ...
      draft.step===4&&v4h(React.Fragment,null,v4h('p',null,'원했던 것과 실제로 한 행동을 나란히 보고, 가장 가까운 답을 골라요.'),v4h('div',{className:'v4-reflection-recap'},v4h('div',null,v4h('small',null,'내가 원했던 것'),v4h('p',null,draft.need||'앞의 욕구 질문에서 적을 수 있어요.')),v4h('div',null,v4h('small',null,'내가 한 행동'),v4h('p',null,draft.action||'앞의 행동 질문에서 적을 수 있어요.'))),v4h('div',{className:'v4-reflection-options','aria-label':'돌아보기 선택'},V4StudentTools.reflections.map(r=>v4h('button',{type:'button',key:r.value,disabled:busy,'aria-pressed':draft.helpful===r.value,onClick:()=>update('helpful',r.value)},v4h('strong',null,r.value),v4h('span',null,r.example)))),draft.helpful&&!V4StudentTools.reflections.some(r=>r.value===draft.helpful)&&v4h('p',{className:'v4-callout'},'전에 남긴 돌아보기: '+draft.helpful),v4h('p',{className:'v4-fine'},'어떤 답도 괜찮아요. 나를 평가하는 점수가 아니에요.'))),
     v4h('div',{className:'v4-activity-footer'},draft.step>0&&btn('이전 질문',()=>go(draft.step-1)),draft.step<4?btn('다음 질문 →',()=>go(draft.step+1),'v4-primary'):btn(busy?'저장 중…':'내 생각 저장하기',save,'v4-primary',!any||(!fiction&&load!=='ready'))),v4h('p',{className:'v4-fine'},'떠오르지 않는 질문은 다음으로 넘어가도 돼요. 섬을 둘러보는 동안 이어 쓸 수 있어요.')),
...
function V4Archive({me,entries,data,change,onLeavePlace,onUpdate,onDelete,onLegacy,readOnly=false,onInspect}){
 ...
 function record(e){const w=V4ActivityModel.words(e,data.choices);return v4h('article',{className:'v4-journal-card',key:e.ts},v4h('header',null,v4h('span',{className:'v4-eyebrow'},e.date),v4h('span',{className:'v4-private-tag'},readOnly?'학생의 일기':'나의 일기')),
  ...
  v4h('div',{className:'v4-actions'},v4h('button',{disabled:busy,onClick:()=>readOnly?onInspect(e.ts):act(async()=>{await change({type:'selectEntry',ts:e.ts});onLeavePlace('review','이 글을 골랐어요. 관찰대가 빛나요.');})},'이 마음 살펴보기'),!readOnly&&(editing===e.ts?v4h(React.Fragment,null,v4h('button',{className:'v4-primary',disabled:busy||!raw.trim(),onClick:()=>act(async()=>{await onUpdate({...e,text:raw,analysis:v4Analyze(raw)});V4ActivityDrafts.delete(me.schoolCode+':'+me.id+':'+e.ts);setEditing(null);},'글을 수정했어요. 예전 대화 연결도 정리했어요.')},busy?'저장 중…':'수정 저장'),v4h('button',{disabled:busy,onClick:()=>setEditing(null)},'수정 취소')):v4h('button',{disabled:busy,onClick:()=>{setEditing(e.ts);setRaw(e.text);}},'원문 수정')),!readOnly&&v4h('button',{className:'v4-text',disabled:busy,onClick:()=>setRemove(e.ts)},'삭제')),
 ...
 return v4h(React.Fragment,null,v4h(V4ActivityHeading,{kind:'archive',kicker:'기록 보관함 · 나에게 남긴 이야기',title:'지난 마음을 다시 만나요.',copy:'달력 속 하루를 펼치거나, 여러 날의 기록을 함께 돌아봐요.'}),
  ...
  !readOnly&&v4h(V4ReceivedMessages,{me}),!readOnly&&v4h('div',{className:'v4-archive-extra'},v4h('button',{className:'v4-text',onClick:onLegacy},'세부 분석·기록 도구')),
