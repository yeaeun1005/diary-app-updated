...
`sounds/` 의 mp3 셋. 앱은 기본 음소거이고 3D 화면의 "소리" 버튼으로 켠다. CC0 등 라이선스가 명확한 것만 쓴다. (출처는 채워 넣을 것 — 2026-09-14)

| 파일 | 쓰임 | 길이 | 원본 이름 | 만든 사람 | 출처 URL | 라이선스 | 손본 것 |
|---|---|---|---|---|---|---|---|
| `sounds/piano_loop.mp3` | 배경음. 내 섬·친구 섬·바다·광장에서 루프(음량 0.30) | 56초 | | | | | |
| `sounds/step.mp3` | 발걸음. 걷는 동안 루프, 멈추면 끔(음량 1.0) | 11초 | | | | | |
| `sounds/woosh.mp3` | 화면 전환. 잠수·광장·바다로 갈 때 한 번(음량 0.10) | 1.5초 | | | | | |
